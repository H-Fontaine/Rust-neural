...
fn main() {
    ...
    */
    ...
    let (ones_images, _ones_labels) = select_specific_numbers(images_test, label_test, vec![0]);
    let mut generative_network = AdversarialNetworks::new(vec![1, 30, 784], vec![784, 10, 2], learning_rate, &mut rng, &StandardNormal);
    generative_network.training(ones_images, 1000, 20, &mut rng, &Open01);
    let test = generative_network.generate(1, &mut rng, &Open01);

    display(&test, &vec![1], 0);
}